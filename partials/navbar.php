<!-- Top wrapper -->
<div class="navbar-wrapper2 navbar-fixed-top">
  <div class="container">
    <div class="navbar mtnav">

      <div class="container offset-3">
        <!-- Navigation-->
        <div class="navbar-header">
          <button data-target=".navbar-collapse" data-toggle="collapse" class="navbar-toggle" type="button">
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="igecon-bar"></span>
          </button>
          <a href="/nimun/index.php" class="navbar-brand"><img src="/nimun/nimun.png" alt="NIMUN '17" class="logo"/></a>
        </div>

        <div class="navbar-collapse collapse">
        <ul class="nav navbar-nav navbar-right">
            <li class="dropdown">
              <a href="/nimun/index.php">HOME</a>
            </li>

            <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">ABOUT</a>
            <ul class="dropdown-menu posright-0">
              <li>
                <div class="row" style="width:250px;">
                  <ul class="droplist col-md-12">
                    <li><a href="/nimun/about/muns/index.php">What is NIMUN?</a></li>
                    <li><a href="/nimun/about/tourism/index.php">Pakistan: Culture &amp Tourism</a></li>
                    <li><a href="/nimun/about/islamabad/index.php">Islamabad - The Capital</a></li>
                    <li><a href="/nimun/about/nust/index.php">NUST</a></li>
                  </ul>
                </div>
              </li>
            </ul>
            </li>

            <li class="dropdown">
              <a data-toggle="dropdown" class="dropdown-toggle" href="#">NIMUN'17</a>
              <ul class="dropdown-menu posleft-0">
                <li>
                  <div class="row" style="width:200px;">
                    <ul class="droplist col-md-12">
                      <li><a href="/nimun/nimun/nimun14/index.php">Introduction</a></li>
                      <li><a href="/nimun/nimun17/conferenceTheme/index.php">Conference Theme</a></li>
                      <li><a href="/nimun/nimun17/historyOfNIMUN/index.php">History of NIMUN</a></li>
                    </ul>
                  </div>
                </li>
              </ul>
            </li>


            <li class="dropdown">
              <a data-toggle="dropdown" class="dropdown-toggle" href="#">OUR TEAM</a>
              <ul class="dropdown-menu posleft-0">
                <li>
                  <div class="row" style="width:200px;">
                    <ul class="droplist col-md-12">
                      <li><a href="/nimun/nimun/nimun14/index.php">Executive Council</a></li>
                      <li><a href="/nimun/nimun/nimun15/index.php">Directorate</a></li>
                    </ul>
                  </div>
                </li>
              </ul>
            </li>


          <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">CONFERENCE</a>
            <ul class="dropdown-menu posleft-0">
              <li>
                <div class="row" style="width:670px;">
                  <ul class="droplist col-md-3">
                    <li class="dropdown-header">Details</li>
                    <li><a href="/nimun/conference/details/committee-directors/index.php">Committee Directors</a></li>
                    <li><a href="/nimun/conference/details/conference-fees/index.php">Conference Schedule</a></li>
                    <li><a href="/nimun/conference/details/conference-schedule/index.php">Conference Fees</a></li>
                  </ul>
                  <ul class="droplist col-md-3">
                    <li class="dropdown-header">Committees</li>
                    <li><a href="/nimun/conference/committees/general-assembly/index.php">General Assembly</a></li>
                    <li><a href="/nimun/conference/committees/general-assembly/index.php">Security Council</a></li>
                    <li><a href="/nimun/conference/committees/programmes-and-funds/index.php">Programmes and Funds</a></li>
                    <li><a href="/nimun/conference/committees/regional-bodies/index.php">Regional Bodies</a></li>
                    <li><a href="/nimun/conference/committees/specialized-committees/index.php">Specialized Committees</a></li>
                  </ul>
                  <ul class="droplist col-md-3">
                    <li class="dropdown-header">Preparation</li>
                    <li><a href="/nimun/conference/preparation/guides-and-downloads/index.php">Guides and Downloads</a></li>
                    <li><a href="/nimun/conference/preparation/rules-of-procedure/index.php">Rules of Procedure</a></li>
                  </ul>
                  <ul class="droplist col-md-3">
                    <li class="dropdown-header">Logistics</li>
                    <li><a href="/nimun/conference/logistics/gettingthere/index.php">Venue</a></li>
                    <li><a href="/nimun/conference/logistics/accomodation/index.php">Accomodation</a></li>
                    <li><a href="/nimun/conference/logistics/venue/index.php">Getting There</a></li>
                  </ul>
                </div>
              </li>
            </ul>
          </li>


          <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">REGISTRATION</a>
            <ul class="dropdown-menu posleft-0">
              <li>
                <div class="row" style="width:500px;">
                  <ul class="droplist col-md-6">
                    <li class="dropdown-header">Registration Type</li>
                    <li>
                      <a href="/nimun/chairapplication/index.php">Chair Application</a>
                    </li>
                    <li><a href="login">Local</a></li>
                    <li><a href="login">International</a></li>
                    <li class="dropdown-header">Have an account?</li>
                    <li><a href="login">Login</a></li>
                  </ul>
                  <ul class="droplist col-md-6">
                    <li class="dropdown-header">More Details</li>
                    <li><a href="list.html">How to register</a></li>
                    <li><a href="list2.html">Registration Timeline</a></li>
                  </ul>
                </div>
              </li>
            </ul>
          </li>





          <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">INTERNATIONAL</a>
            <ul class="dropdown-menu posleft-0">
              <li>
                <div class="row" style="width:150px;">
                  <ul class="droplist col-md-12">
                    <li><a href="/nimun/international/visa/index.php">Visa</a></li>
                    <li><a href="/nimun/international/registeration/index.php">Registration</a></li>
                    <li><a href="/nimun/international/guide/index.php">Guide</a></li>
                  </ul>
                </div>
              </li>
            </ul>
          </li>




          <li class="dropdown">
            <a data-toggle="dropdown" class="dropdown-toggle" href="#">MEDIA</a>
            <ul class="dropdown-menu posleft-0">
              <li>
                <div class="row" style="width:200px;">
                  <ul class="droplist col-md-12">
                    <li><a href="/nimun/international/visa/index.php">Publications &amp Press</a></li>
                    <li><a href="/nimun/international/registeration/index.php">Gallery</a></li>
                  </ul>
                </div>
              </li>
            </ul>
          </li>


          <li><a href="/nimun/contactus/index.php">CONTACT US</a></li>

        </ul>
      </div>
      <!-- /Navigation-->
      </div>

  </div>
  </div>

</div>
<!-- /Top wrapper -->
